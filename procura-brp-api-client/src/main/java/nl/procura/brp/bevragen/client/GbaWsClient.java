...
public class GbaWsClient {

  private final WebClient client;

  @Builder
  public GbaWsClient(GbaWsPersonListClientConfig config) {
  ...
  public GbaWsRestTablesResponse getTables(GbaWsRestTablesRequest request) {
    return client
        ...
        .body(BodyInserters.fromValue(request))
        .retrieve()
        .onStatus(HttpStatus::isError, clientResponse -> error(new GbaWsException(clientResponse.statusCode().value())))
        .bodyToMono(GbaWsRestTablesResponse.class)
        .block();
