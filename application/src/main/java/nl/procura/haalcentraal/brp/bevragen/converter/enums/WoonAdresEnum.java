...
@AllArgsConstructor
@Getter
public enum WoonAdresEnum {

  W(SoortAdresEnum.WOONADRES),
  B(SoortAdresEnum.BRIEFADRES);

  private SoortAdresEnum type;

  public static SoortAdresEnum fromCode(final String code) {
    return Arrays.stream(values())
        .filter(i -> i.name().equals(code))
        .findFirst()
        .orElseThrow(() -> new IllegalArgumentException("Onbekende code : " + code)).getType();
  }
